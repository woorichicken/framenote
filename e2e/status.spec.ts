...
test("에이전트가 바꾼 상태가 화면에 즉시 나타난다", async ({ page }) => {
  // 실행: 에이전트가 그 메모를 반영됨으로 바꾸도록 서버에 요청한다.
  // 기대: 저장 파일의 상태가 바뀌고, 열려 있는 창의 그 메모 카드가 새로 고침 없이 반영됨으로 바뀐다.
  covers(
    "보내기 직후 상단과 메모 카드에 동시에 작업중이 표시된다",
  );
  // 실행: 보내기를 누르고 화면을 그대로 둔다.
  // 기대: 창 위쪽 한 줄에 작업이 도는 중이라는 표시와 그 메모의 식별자가 나타나고, 동시에 목록의 그 메모 카드에도 작업중 표시가 나타난다.
...
test("재렌더가 실패하면 작업중에 머물지 않고 사유가 보인다", async ({ page }) => {
  // 실행: 재렌더가 오류로 실패한다.
  // 기대: 그 메모가 작업중에 머무르지 않고 실패로 표시되며 실패 사유가 화면에 보인다.
...
test("실패 메모 다시 보내기가 대기 중인 초안을 끌고 나가지 않는다", async ({ page }) => {
  // 실행: 실패 메모에서 다시 보내기를 누른다.
  // 기대: 그 메모만 보냄이 되어 즉시 새 묶음으로 나간다.
...
test("새 렌더본이 오면 미완 메모가 낡음이 되고 반영됨은 넘어가지 않는다", async ({ page }) => {
  ...
  // 실행: 에이전트가 그 메모를 고치고 재렌더해 렌더본 B가 된다.
  // 기대: 고쳐진 메모는 반영됨이 되고 낡음으로 표시되지 않는다.
  // 실행: 영상이 렌더본 B로 교체된다.
  // 기대: 그 2건이 낡음으로 표시되고 목록에서 구분되어 보인다.
  // 실행: 에이전트가 소스를 고치고 미리보기 렌더를 완료한다.
  // 기대: 같은 탭의 영상이 새 렌더본으로 바뀌고 창 위 렌더본 식별자가 갱신된다.
...
test("반영됨 메모를 고르면 그 구간으로 이동하고 해결됨으로 닫을 수 있다", async ({ page }) => {
  ...
  // 실행: 목록에서 그 메모를 고른 뒤 해결됐다고 표시한다.
  // 기대: 메모를 고르는 순간 재생 위치가 그 메모의 구간 시작으로 이동한다.
  // 실행: 에이전트가 재렌더를 완료해 영상이 교체된다.
  // 기대: 교체 후에도 재생 위치가 632프레임이다.
...
test("닫은 메모는 이후 보내기 묶음에서 빠진다", async ({ page }) => {
  // 실행: 나머지 메모를 수정한 뒤 보내기를 누른다.
  // 기대: 보내기 묶음에 닫힘 메모가 포함되지 않고 건수가 2다.
...
test("고쳐지지 않은 메모를 다시 열면 새 렌더본 기준으로 되살아난다", async ({ page }) => {
  // 실행: 그 메모를 다시 열기로 표시한다.
  // 기대: 상태가 보냄으로 돌아가고, 그 메모의 렌더본이 지금 보고 있는 새 렌더본으로 갱신된다.
...
test("낡은 메모를 다시 찍으면 글과 이미지는 남고 좌표만 갱신된다", async ({ page }) => {
  ...
  // 실행: 그 메모에서 다시 찍음을 고르고 새 렌더본에서 구간과 네모를 다시 잡는다.
  // 기대: 같은 메모 식별자가 유지된 채 구간과 좌표와 렌더본 세 값이 모두 갱신되고 상태가 초안이 된다.
...
test("보내기 전 메모는 글과 구간을 고칠 수 있다", async ({ page }) => {
  ...
  // 실행: 그 메모의 글을 바꾸고, 구간을 다른 값으로 다시 잡고, 이미지를 1장 더 붙인 뒤 저장한다.
  // 기대: 세 변경이 모두 반영되어 다시 읽을 때 보이고, 메모 상태는 초안 그대로다.
...
test("한 프레임 이동이 정확히 한 프레임씩 움직인다", async ({ page }) => {
  // 실행: 다음 프레임으로 한 칸 이동을 10번, 이전 프레임으로 한 칸 이동을 10번 한다.
  // 기대: 매 이동마다 프레임 번호가 정확히 1씩 변하고, 20번 뒤 처음 위치로 돌아온다.
  await seek(page, 100);
