...
test("메모 한 건이 사람 개입 없이 수정·재렌더까지 완주한다", async ({ page }) => {
  // 실행: 영상 위에 네모를 그려 메모를 하나 쓰고 보내기를 누른 뒤, 사람은 아무것도 하지 않고 기다린다.
  // 기대: (1) 에이전트에게 알림이 도착하고 (2) 소스 파일이 수정되고 (3) 미리보기 렌더가 돌고 (4) 같은 창의 영상이 새 렌더본으로 바뀌고 창 위 렌더본 식별자가 달라진다.
  covers(
...
test("화면에 보이는 프레임 번호와 도구가 기록한 번호가 같다", async ({ page }) => {
  // 실행: 임의의 프레임 다섯 곳(키프레임이 아닌 자리를 포함)으로 이동하고, 각 지점에서 영상에 보이는 숫자와 도구가 표시한 프레임 번호를 비교한다.
  // 기대: 다섯 곳 모두 두 숫자가 정확히 같다.
  covers("첫 프레임과 마지막 프레임의 번호가 어긋나지 않는다");
  // 실행: 맨 처음 프레임과 맨 마지막 프레임으로 각각 이동한다.
  // 기대: 도구가 표시하는 번호가 0과 1799다.
...
test("Remotion 프로젝트 영상은 씬 이름과 경계가 보인다", async ({ page }) => {
  // 실행: 그 영상을 연다.
  // 기대: 영상 종류가 remotion으로 표시되고, 스크러버에 씬이 구간으로 나뉘어 씬 이름이 보인다.
